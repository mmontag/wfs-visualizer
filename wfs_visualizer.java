...
public class sketch_mar01a_wfs_simulation8 extends PApplet {
...
public void initArray() {
    ...
    for(int i = 0; i < num; i++) {
        ...
        if (randomize==1) {
            float rnd = 0.50f; //randomization factor 10-50%
    ...
    for (int i = xpos/upsample; i< wd; i++) {
        for (int j = 0; j < hd; j++) {
            ...
            for (int e = 0; e < loudspeakers.size(); e++) {
                ...
                switch(signaltype) {
                case 0: //sinusoid
                    f += PApplet.parseInt( 128/loudspeakers.size() + 4048 * 1/d * u.a * u.att * sin(2*PI * u.w * ( d - t - u.p )  ) );
                    break;
                case 1: //noise
                    f += PApplet.parseInt( 128/loudspeakers.size() + 2048 * 1.5f/d * u.a * u.att * (noise(2 * u.w * ( d - t - u.p )  )-.5f) );
                    break;
                case 2: //saw
                    float impulse = map((-u.w * ( d - t - u.p ))%1, 0,1, -1,1);
                    f += PApplet.parseInt( 128/loudspeakers.size() + 2048 * 1/d * u.a * u.att * impulse );
                    break;
                }
            }
            if(f < 0) f = 0;
            ...
            if(primarywave ==1) {
                ...
                switch(signaltype) {
                case 0: //sinusoid
                    r += PApplet.parseInt( 124 + 2548/pow(d,.707f) * sin(2*PI / wavelength * ( d - t )  ) );
                    break;
                case 1: //noise
                    r += PApplet.parseInt( 64 * 2048/d  * (noise(2 / wavelength * ( d - t )  )-.5f) );
                    break;
                case 2: //saw
                    float impulse = map((-1/wavelength * ( d - t ))%1, 0,1, -1,1);
                    r += PApplet.parseInt( 128 * 255/d * impulse );
                    break;
                }
                if(r < 0) r = 0;
                if(r > 255) r = 255;
...
public void keyPressed() {
    ...
    if(key == 'q' && taperwidth > 0.05f) taperwidth-=.05f; //adjust array tapering profile
    if(key == 'w' && taperwidth < .5f) taperwidth+=.05f;  
    if(key == '1' && upsample > 1) { 
        upsample -= 1; 
        initBuffer();
    } //adjust detail level
    ...
    static public void main(String args[]) {
        PApplet.main(new String[] { "--bgcolor=#d4d0c8", "sketch_mar01a_wfs_simulation8" });
    }
}
